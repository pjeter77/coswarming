setwd("~/Dropbox/Daubentonii_coswarming/data/final/github")
...
individuals <- read.csv("KWdata.csv")
...
##################################################################
# Interactions during season                                     #
##################################################################

data <- read.csv("GAMdata.csv")
data$sexage <- as.factor(data$sexage)

mod_gam = gam(interactions ~ s(daytopart, by = sexage, k = 10), 
              family = poisson(link = "log"), method = "REML", data = data)
gam.check(mod_gam)
summary(mod_gam)

draw(mod_gam, constant = mean(data$interactions), select = 1, scales = "fixed") + 
  xlim(-50,100) + ylim(2,7) +
  theme_ipsum()
draw(mod_gam, constant = mean(data$interactions), select = 2, scales = "fixed") + 
  xlim(-50,100) + ylim(2,7) +
  theme_ipsum()
draw(mod_gam, constant = mean(data$interactions), select = 3, scales = "fixed") + 
  xlim(-50,100) + ylim(2,7) +
  theme_ipsum()
